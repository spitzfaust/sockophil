...
public:
    MenuTest() {
        ...
        this->connected_on = "127.0.0.1:1337";
...
TEST_F(MenuTest, test_get_connected_on) {
    EXPECT_STREQ(menu->get_connected_on().c_str(), connected_on.c_str());
    EXPECT_STRNE(menu->get_connected_on().c_str(), ip_address.c_str());
    EXPECT_STRNE(menu->get_connected_on().c_str(), "");
}
